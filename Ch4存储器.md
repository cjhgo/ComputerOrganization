...
6个盘片,上下外侧为保护面,共有10个盘面可作为记录面
对应10个磁头
磁头具有浮动特性,盘面高速旋转时,盘面形成的高速气流微微托起磁头
...
直接存取,只要知道信息所在盘面,磁道和扇区的位置,
磁头便可以直接找到其位置并读写

**柱面**
任何时刻各磁头都位于距圆心相等距离的磁道上,这组磁道称为一个柱面


半导体盘
用半导体材料制成`盘`,功能上模拟硬盘
####循环冗余检验码
发现并纠正信息在存储或传送过程中连续出现的多位错误代码
CRC在磁介质存储器和计算机网络中得到广泛应用
模2运算,不考虑进位和借位

+ $模2加,模2减,a\pm b (mod2)结果是相等的$
	- 实践中,可以直接对a,b按异或来算
	- $本来0-1=-1,要模2来算,结果就变成了1\\\
	所以,-1的模2是1,????不懂了$
+ 模2乘,对部分积,按模2求和,
部分积的每一位求和时,都按模2加来算,不进位
+ 模2除,
	- 计算CRC码时,用的正是模2除法
	模2除的时候,求部分余数用模2减
	(可以用异或来算)
	...
	- 上商原则,
	部分余数首位为1,上商1
	部分余数首位为0,上商0(上商为0,减数也就是0,而不是除数,,)

CRC编码方法

$n位有效信息D_{n-1}D_{n-2}..D_2D_1D_0,\\\
可以用多项式M(x)=D_{n-1}x^{n-1}+D_{n-2}x^{n-2}+..D_2x^{2}+D_1x^{1}+D_0x^{0}来表示\\\
数位称为多项式的系数,数位为0,对应项不出现,数位为1对应项出现\\\
例如有效信息1100,M(x)=x^3+x^2\\\
将信息码左移k位,得M(x)\cdot x^k,对应D_{n-1+k}D_{n-2+k}..D_{2+k}D_{1+k}D_{0+k}
\underbrace{00\cdots 00}_k\\\
CRC码就是用多项式M(x)\cdot x^k 除以生成多项式G(x)(产生校验码的多项式)\\\
所得余数 填到k位0中,作为校验位,为了得到k位校验码,G(x)必须是k+1位,\\\
反之,\underline{G(x)是m位,则可以得到m-1位的校验码}\\\
设模2除结果是这样的,M(x)\cdot x^k=Q(x)\cdot G(x)+R(x)\\\
则M(x)\cdot x^k+R(x)=[Q(x)\cdot G(x)+R(x)]+R(x)\\\
=[Q(x)\cdot G(x)]+(R(x)+R(x))\\\
=Q(x)\cdot G(x),模2和,(按模2求和,R(x)+R(x),各位都一样,结果自然是0)
这说明CRC码是一个可被G(x)除尽的数码,\\\
所以如果得到的CRC码不能被除尽就说明出现了错误
$

译码和纠错
对得到的CRC,用G(X)去除,
若余数为0,则没出错
若余数不为0,则余数是几,就是哪一位除了错
比如,余数是010,那就是第2位除了错[(7,4)码]
