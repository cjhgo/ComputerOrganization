...
加了屏蔽功能的排队器解决了按优先级中断嵌套的问题*屏蔽问题*
(不加屏蔽功能,则随意嵌套,,)
+ 三次屏蔽
	- cpu中断允许屏蔽,控制能否响应中断
	- 接口处屏蔽,控制能不能发中断请求,好比8259的OCW1
	- 排队器处屏蔽,控制中断嵌套优先级别,好比8259的OCW3


#####屏蔽触发器和屏蔽字
![具有屏蔽功能的排队器][4]
[4]:http://cjhgo.sinaapp.com/CS/ComputerOrganization/images/chainqueueofmask.gif

集中设在CPU内的排队器加上屏蔽条件,组成上图所示的具有屏蔽功能的排队器
工作原理:
$
INTP_1=INTR_1 \cdot \overline{MASK_1}\\\
INTP_i=\overline{INTP_1}\cdot\overline{INTP_2}
\cdots\overline{INTP_{i-1}}\cdot INTR_i \cdot \overline{MASK_i}(i > 1)
\\\
=\overline{(INTP_{1}+INTP_{2}+\cdots+INTP_{i-1})}\cdot INTR_i \cdot \overline{MASK_i},
记为\overline{A}\cdot INTR_i\\\
$
可见,

+ $任一级中断都可以用MASK_i屏蔽掉$
+ $当且仅当优先级高的中断没有排队输出(A=0),且未被屏蔽时(MASK_i=0),中断请求才能发出$


#####屏蔽技术改变优先级

+ 通过软件控制程序中断接口电路中的屏蔽触发器MASK,可以灵活处置某一个设备能否发中断请求
+ 响应优先级决定了多个中断同时请求时优先响应哪个,是硬件线路已经设置好的,不便于改动,
	- 所谓硬件线路已设置好的,是指,在链式排队器的线路中,
	先天地,固定好了CPU有一个响应的顺序,这就是固定的响应优先级
		+ 引入了屏蔽字之后,相当于在排队器处设置一个门槛,控制被屏蔽的中断直接无法参与排队
		过了这个门槛的中断还是按照响应优先级排队
			- 原始的屏蔽字是在中断嵌套时把同级和低级中断屏蔽掉,使其无法进入排队,自然无法中断当前的中断服务
			- 修改原始屏蔽字使得响应级别低的中断有机会进入排队,而正如上边说的,过了门槛,还是按硬件响应
			所以,修改原始屏蔽字,改不了响应优先级,改的是处理优先级
				+ 改变了处理优先级,是指,级别低的比级别高的先处理完
				不修改的情况下,级别高低决定了处理完的先后顺序
+ 通过软件控制屏蔽字,可以灵活的改变中断嵌套时的优先级(处理优先级)
开中断后中断嵌套时,还是按硬件的优先级响应
(因为既然这些中断请求都进入了中断嵌套竞争cpu响应,则在屏蔽方面是无差别的,
按照公式,自然是响应优先级高的优先响应
**就是说,虽然改变了屏蔽字,但都改变了,所以还是按原先的优先级响应**
)
...
+ 断点保存在堆栈中,按相反顺序出栈就可以返回到程序间断处
+ 断点保存到0号地址单元中,中断嵌套会冲掉之前的断点,
所以应在保存现场之后开中断之前这个时间里将0地址单元内容转移,??????????
	

#####cpu执行程序的轨迹/多重中断处理示意图
**回到主程序**
